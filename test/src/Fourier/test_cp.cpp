...
int main(int argc, char ** argv) {
  ...
  magI -= Min;
  magI.convertTo(dist_8U,CV_8U,255.0/(Max-Min));

  imshow("Input Image"       , I   );    // Show the result
  cv::imshow("IDF", dist_8U);
//  imshow("spectrum magnitude", magI);

 
  cv::Mat dft_src;
  double min, max;
  idft(dist_8U, dft_src);
  split(dft_src, mv);
  minMaxLoc(mv[0], &min, &max);
  cv::Mat idft_img = Mat(mv[0]*1.0/max, Rect(0, 0, src_cols, src_rows));
  cv::imshow("IDFT", idft_img);
  waitKey();
